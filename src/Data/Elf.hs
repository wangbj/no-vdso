...
-- | Assumes the given section is a symbol table, type SHT_DYNSYM
-- (guaranteed by parseSymbolTables).
getDynamicSymbolTableEntries :: Elf -> ElfSection -> [ElfSymbolTableEntry]
getDynamicSymbolTableEntries e s = go decoder (L.fromChunks [elfSectionData s])
  where
    link   = elfSectionLink s
    strtab = lookup (fromIntegral link) (zip [0..] (elfSections e))
    decoder = runGetIncremental (getSymbolTableEntry e strtab)
...
symbolTableSections e = filter ((\x -> x == SHT_SYMTAB || x == SHT_DYNSYM) . elfSectionType) (elfSections e)
